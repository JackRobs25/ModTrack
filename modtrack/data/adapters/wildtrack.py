"""WildTrack dataset adapter used by the ModTrack evaluation pipeline."""
...
WILDTRACK_IMAGE_EXTS = (".png", ".jpg", ".jpeg", ".bmp", ".tif", ".tiff")
WILDTRACK_CAMERAS = ["C1", "C2", "C3", "C4", "C5", "C6", "C7"]
WILDTRACK_CAMERA_CALIBRATIONS = ["CVLAB1", "CVLAB2", "CVLAB3", "CVLAB4", "IDIAP1", "IDIAP2", "IDIAP3"]
_GRID_WIDTH = 480
...
def _load_intrinsics(intrinsic_dir: Path) -> Dict[str, np.ndarray]:
    ...
    for file in intrinsic_dir.glob("*"):
        if not file.is_file():
            continue
        try:
...
def _load_extrinsics(extrinsic_dir: Path) -> Dict[str, np.ndarray]:
    ...
    for file in extrinsic_dir.glob("*"):
        ...
        if rvec_elem is None or tvec_elem is None:
            continue
        rvec = np.array([float(x) for x in rvec_elem.text.split()], dtype=np.float64)
        tvec = np.array([float(x) for x in tvec_elem.text.split()], dtype=np.float64)
        # Convert Rodrigues vector to rotation matrix (world->camera)
        R_wc = _rodrigues(rvec)
        # WildTrack extrinsics are provided in centimetres; convert to metres for consistency.
        t_wc = (tvec / 100.0).astype(np.float32)
        # Convert to camera->world transform
        T_cw = np.eye(4, dtype=np.float32)
        T_cw[:3, :3] = R_wc.T
        T_cw[:3, 3] = (-R_wc.T @ t_wc).astype(np.float32)
        extrinsics[file.stem.upper()] = T_cw
    return extrinsics
...
def _load_calibration(root: Path) -> Dict[str, Tuple[np.ndarray, np.ndarray]]:
    ...
    if not intr_dir.is_dir() or not ext_dir.is_dir():
        raise FileNotFoundError("WildTrack calibration directories not found under 'calibrations'.")
    intrinsics = _load_intrinsics(intr_dir)
    extrinsics = _load_extrinsics(ext_dir)
    calibs: Dict[str, Tuple[np.ndarray, np.ndarray]] = {}
    for i in range(len(WILDTRACK_CAMERAS)):
        key = WILDTRACK_CAMERA_CALIBRATIONS[i]
        K = intrinsics.get(f"INTR_{key}")
        T = extrinsics.get(f"EXTR_{key}")
        if K is None or T is None:
            raise FileNotFoundError(f"Calibration missing for camera '{WILDTRACK_CAMERAS[i]}'.")
        calibs[WILDTRACK_CAMERAS[i]] = (K.astype(np.float32), T.astype(np.float32))
    return calibs
...
def _camera_directory(root: Path, camera: str) -> Path:
    ...
    if not cam_dir.is_dir():
        raise FileNotFoundError(f"WildTrack camera directory not found for '{camera}'.")
    return cam_dir
...
def iter_wildtrack_frames(
    ...
    """
    Iterate over WildTrack frames for a given camera.
    ...
    step = max(1, every_n)
    emitted = 0
    images = sorted(p for p in cam_dir.iterdir() if p.suffix.lower() in WILDTRACK_IMAGE_EXTS)
    cam_index = WILDTRACK_CAMERAS.index(cam_key)
